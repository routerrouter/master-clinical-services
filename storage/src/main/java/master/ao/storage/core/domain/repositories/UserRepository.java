...
public interface UserRepository extends JpaRepository<User, UUID>, JpaSpecificationExecutor<User> {
    ...
    @Modifying
    @Query(value = "INSERT INTO tb_users (user_id, creation_date, email, full_name, group_id, username)\n" +
    ...
    @Modifying
    @Query(value = "UPDATE  tb_users set email=?3, full_name=?4, username=?5,last_update_date=?2 where user_id=?1", nativeQuery = true)
